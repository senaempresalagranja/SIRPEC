...
<!-- ============================================================================== -->
		<h1 class="help-block text-center ">Ingresar Especie</h1>
	</div>
		<div class="container">
				...
				<div class="col-xs-12 col-sm-2 col-md-2 col-lg-2">
					<div class="form-group">
						<div  id="elemento_nombre">
							...
							<input type="text" placeholder="" name='nombre_cientifico' id="nombre_cientifico" class="form-control">
...
<br>
				...
				<div class="col-md-10">
                <div class="col-md-5"></div>
                <a onclick="enviar()" title="Registrar Especie" class="icon-button floppy-o"><i class="fa fa-floppy-o"></i><span></span></a>
				<a onclick="nuevo_registro()" title="Nuevo Registro" class="icon-button share"><i class="fa fa-share"></i><span></span></a>
				<a onclick="Actualizar()" title="Actualizar Especie" class="icon-button refresh"><i class="fa fa-refresh"></i><span></span></a>
				<a onclick="consultar_modal()" title="Consultar Especie" class="icon-button search"><i class="fa fa-search"></i><span></span></a>
				<a title="Exportar Especie Excel" onclick="exportar_excel()" class="icon-button file-excel-o"><i class="fa fa-file-excel-o"></i><span></span></a>
                <a onclick="exportar()" title="Exportar Especie PDF" class="icon-button file-pdf-o"><i class="fa fa-file-pdf-o"></i><span></span></a>
                </div>
...
<h1 class="modal-title text-center">Consultar Especie</h1>
...
<div class="container-fluid">
	<div class="row">
		...
		<div class="col-md-12">
			<div class="form-group"><label for="">Buscar Especie</label>
...
echo "<option name='' id='' value='$fila[0]'>$fila[1]---$fila[2]</option>";
}
...
$("#contenedor").load("registrar_especie.php",{nombre:nombre,nombre_cientifico:nombre_cientifico})

}

function consultar() {

var id_especie=document.getElementById("id_especie").value;

$("#contenedor").load("consultar_especie.php",{id_especie:id_especie})
...
$("#contenedor").load("actualizar_especie.php",{id_especie:id_especie,nombre:nombre,nombre_cientifico:nombre_cientifico})

}

function nuevo_registro() {
	var nombre=document.getElementById("nombre").value="";
	var id_especie=document.getElementById("id_especie").value="Selecciona";
var nombre_cientifico=document.getElementById("nombre_cientifico").value="";
}
...
swal({
  title: "Advertencia",
  text: "Desea Exportar Esta Especie?",
  type: "warning",
  showCancelButton: true,
  confirmButtonColor: "#16e528",
  confirmButtonText: "Solo Este Fomulario",
  cancelButtonText: "No, Todas las Especies",
  closeOnConfirm: true,
  closeOnCancel: true
},
function(isConfirm){
  if (isConfirm) {
var formulario=document.getElementById('formulario').action="exportaciones/exportar_especie.php";

$("#formulario").submit();
 
  } else {
  	var formulario=document.getElementById('formulario').action="exportaciones/exportar_todas_especies.php";
...
function(isConfirm){
  if (isConfirm) {
var formulario=document.getElementById('formulario').action="exportaciones/exportar_especie_excel.php";

$("#formulario").submit();
 
  } else {
  	var formulario=document.getElementById('formulario').action="exportaciones/exportar__todas_especie_excel.php";
