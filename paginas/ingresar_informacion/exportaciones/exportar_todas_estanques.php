...
$pdf->SetTitle('SIRPEC');
$pdf->SetSubject('GUILLERMOS');
$pdf->SetKeywords('EXPORTAR ESTANQUES');
...
$sql="SELECT finca.nombreFinca, `numeroEstanque`, `totEspejoAgua`, `profundPromedio`, `volumAgua`, `observacion` FROM `estanque` INNER JOIN finca ON estanque.idFinca=finca.idFinca";
...
<p></p>
<h1>ESTANQUES SIRPEC</h1>

<table >
			<tr>
				<td><b>Nombre Finca</b></td>
				<td><b>Numero de Estanque</b></td>
				<td><b>Espejo de Agua</b></td>
				<td><b>Profundidad Promedio</b></td>
				<td><b>Volumen Agua</b></td>
...
foreach ($lotes as $row) {
	...
	$nombreFinca=$row["nombreFinca"];
	$numeroEstanque=$row["numeroEstanque"];
	$totEspejoAgua=$row["totEspejoAgua"];
	$profundPromedio=$row["profundPromedio"];
	$volumAgua=$row["volumAgua"];
...
// NOMBRE DEL ARCHIVO
$pdf->Output('FORMULARIO_ESTANQUES.pdf', 'D');
