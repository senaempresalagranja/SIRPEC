...
$pdf->SetKeywords('EXPORTAR MUESTREOS');
...
$sql="SELECT siembra.lote, muestreo.hora, muestreo.fecha, muestreo.numerPeces, `pesoPromedio`, `unidadMedida`, muestreo.observacion FROM `muestreo` INNER JOIN siembra ON muestreo.idSiembra=siembra.idSiembra";
...
foreach ($lotes as $row) {
	...
	$hora=$row["hora"];
	$fecha=$row["fecha"];
	$numerPeces=$row["numerPeces"];
	$pesoPromedio=$row["pesoPromedio"];
...
// NOMBRE DEL ARCHIVO
$pdf->Output('FORMULARIO_MUESTREOS.pdf', 'D');
