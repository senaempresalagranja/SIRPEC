...
$pdf->SetKeywords('EXPORTAR COSECHAS');
...
$sql="SELECT siembra.lote, estanque.numeroEstanque, cosecha.fecha, cosecha.hora, cosecha.numeroPeces,cosecha.pesoPezEntero, cosecha.pesoPezEvicerado, cosecha.observacion FROM `cosecha` INNER JOIN siembra_estanque ON cosecha.idSiembra_Estanque=siembra_estanque.idSiembra_Estanque INNER JOIN siembra ON siembra_estanque.idSiembra=siembra.idSiembra INNER JOIN estanque ON siembra_estanque.idEstanque=estanque.idEstanque";
...
<p></p>
<h1>COSECHAS SIRPEC</h1>

<table >
			<tr>
				<td><b>Lote</b></td>
				<td><b>Numero Estanque</b></td>
				<td><b>Fecha</b></td>
				<td><b>Hora</b></td>
				<td><b>Numero de Peces</b></td>
				<td><b>Peso Pez Entero</b></td>
				<td><b>Peso Pez Evicerado</b></td>
...
foreach ($lotes as $row) {
	...
	$lote=$row["lote"];
	$numeroEstanque=$row["numeroEstanque"];
	$fecha=$row["fecha"];
	$hora=$row["hora"];
	$numeroPeces=$row["numeroPeces"];
	$pesoPezEntero=$row["pesoPezEntero"];
	$pesoPezEvicerado=$row["pesoPezEvicerado"];
...
<table>
				...
				<tr>
<td>'.$lote.'</td>
<td>'.$numeroEstanque.'</td>
<td>'.$fecha.'</td>
<td>'.$hora.'</td>
<td>'.$numeroPeces.'</td>
<td>'.$pesoPezEntero.'</td>
<td>'.$pesoPezEvicerado.'</td>
<td>'.$observacion.'</td>
				</tr>
...
// NOMBRE DEL ARCHIVO
$pdf->Output('FORMULARIO_COSECHAS.pdf', 'D');
